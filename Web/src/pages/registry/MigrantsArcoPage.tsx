...
export function MigrantsArcoPage({ onSessionExpired, user }: Props) {
  ...
  return <section className="workspace-stack"><section className="workspace-panel"><div className="arco-page__heading"><div><h2 className="workspace-panel__title">{t("ARCO rights workspace", "Espacio de derechos ARCO")}</h2><p className="workspace-panel__copy">{t("Signed Access, Rectification, Cancellation, and Opposition requests.", "Solicitudes firmadas de Acceso, Rectificación, Cancelación y Oposición.")}</p></div><button className="session-action session-action--quiet session-action--inline" onClick={() => void load()} type="button">{t("Refresh", "Actualizar")}</button></div>
    ...
    <section className="arco-section"><h2>{t("Pending action", "Acción pendiente")}</h2>{loading ? <p>{t("Loading requests...", "Cargando solicitudes...")}</p> : <ArcoRequestList busyId={busyId} onDecision={requestDecision} onSessionExpired={onSessionExpired} requests={pending} user={user} />}</section>
    <section className="arco-section"><h2>{t("Resolved requests", "Solicitudes resueltas")}</h2><ArcoRequestList busyId={busyId} onDecision={requestDecision} onSessionExpired={onSessionExpired} requests={resolved} user={user} /></section>
    {pendingDecision ? <div aria-labelledby="arco-decision-title" aria-modal="true" className="confirmation-modal" onClick={(event) => { if (event.target === event.currentTarget && busyId === null) setPendingDecision(null) }} onKeyDown={(event) => { if (event.key === 'Escape' && busyId === null) setPendingDecision(null) }} role="dialog">
      <div className="confirmation-modal__surface">
        <div><h3 id="arco-decision-title">{pendingDecision.decision === 'reject' ? t("Reject ARCO request", "Rechazar solicitud ARCO") : t("Approve ARCO request", "Aprobar solicitud ARCO")}</h3><p>{t("Request #", "Solicitud #")}{pendingDecision.request.id} {t("for ", "para ")}{String(pendingDecision.request.registry_entry?.payload_json?.fullName || pendingDecision.request.original_payload_json?.fullName || t(`registration #${pendingDecision.request.registry_entry_id}`, `registro #${pendingDecision.request.registry_entry_id}`))}{t(". This decision requires passkey authentication.", ". Esta decisión requiere autenticación con llave de acceso.")}</p></div>
        <label className="arco-decision-modal__reason">{t("Resolution reason", "Motivo de resolución")}{decisionNeedsReason ? t(" (required)", " (obligatorio)") : t(" (optional)", " (opcional)")}<textarea autoFocus disabled={busyId !== null} maxLength={2000} onChange={(event) => setDecisionReason(event.target.value)} required={decisionNeedsReason} value={decisionReason} /></label>
        {error ? <div className="login-feedback login-feedback--error">{error}</div> : null}
        <div className="confirmation-modal__actions"><button className="session-action session-action--quiet" disabled={busyId !== null} onClick={() => { setPendingDecision(null); setDecisionReason(''); setError(null) }} type="button">{t("Cancel", "Cancelar")}</button><button className="session-action" disabled={busyId !== null || (decisionNeedsReason && !decisionReason.trim())} onClick={() => void decide()} type="button">{busyId !== null ? t("Waiting for passkey...", "Esperando llave de acceso...") : pendingDecision.decision === 'reject' ? t("Reject with passkey", "Rechazar con llave de acceso") : t("Approve with passkey", "Aprobar con llave de acceso")}</button></div>
      </div>
    </div> : null}
  </section></section>
}
