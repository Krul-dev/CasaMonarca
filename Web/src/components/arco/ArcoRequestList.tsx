  ...
  return <div className="arco-list">{requests.map((request) => {
    ...
    return <article className="arco-item" key={request.id}>
      ...
      <div className="arco-item__actions">
        ...
        {request.request_type === 'access' && request.status === 'completed' && request.artifact && !request.artifact.purged_at ? <a className="session-action session-action--quiet session-action--inline" href={getArcoAccessDocumentUrl(request.id)}><AppIcon name="download" />Download Access bundle</a> : null}
      </div>
