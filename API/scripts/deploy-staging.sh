...
show_help() {
  cat <<'EOF'
Open an SSH session to the staging VPS and pull the latest monorepo code there.
...
Optional environment variables:
  ...
  REMOTE_APP_DIR   CasaMonarca monorepo checkout path on the VPS
  REMOTE_API_SUBDIR API project directory inside the monorepo checkout
...
Defaults:
  ...
  REMOTE_PORT=22
  REMOTE_APP_DIR=/home/casamonarca/apps/casamonarca/current
  REMOTE_API_SUBDIR=API
  REMOTE_BRANCH=feature/team-601-3-integrate
  SYNC_STRATEGY=ff-only
  RESET_PERMISSIONS=1

Notes:
  - The remote monorepo itself must already be cloned on the VPS.
  ...
  - This script only updates the Git working tree. Composer, migrations, and cache clears stay manual under REMOTE_APP_DIR/REMOTE_API_SUBDIR.
...
REMOTE_APP_DIR="${REMOTE_APP_DIR:-/home/casamonarca/apps/casamonarca/current}"
REMOTE_API_SUBDIR="${REMOTE_API_SUBDIR:-API}"
REMOTE_BRANCH="${REMOTE_BRANCH:-feature/team-601-3-integrate}"
...
if [[ "${RESET_PERMISSIONS}" == "1" ]]; then
  ...
  api_dir="${REMOTE_APP_DIR}/${REMOTE_API_SUBDIR}"

  if [[ -f "${api_dir}/artisan" ]]; then
    chmod 755 "${api_dir}/artisan"
  fi

  if [[ -d "${api_dir}/scripts" ]]; then
    find "${api_dir}/scripts" -type f -name '*.sh' -exec chmod 755 {} +
  fi

  if [[ -d "${api_dir}/storage" ]]; then
    find "${api_dir}/storage" -type d -exec chmod 755 {} +
    find "${api_dir}/storage" -type f -exec chmod 644 {} +
  fi

  if [[ -d "${api_dir}/bootstrap/cache" ]]; then
    find "${api_dir}/bootstrap/cache" -type d -exec chmod 755 {} +
    find "${api_dir}/bootstrap/cache" -type f -exec chmod 644 {} +
  fi
...
echo "Run Laravel release commands in: ${REMOTE_APP_DIR}/${REMOTE_API_SUBDIR}"
